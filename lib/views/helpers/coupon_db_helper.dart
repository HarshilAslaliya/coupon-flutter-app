...
class CouponDBHelper
{
  ...
  Future<void> initDB() async {
    ...
    await db?.execute(
        "CREATE TABLE IF NOT EXISTS $tableName ($colId INTEGER PRIMARY KEY AUTOINCREMENT, $colCoupon TEXT, $colQuantity INTEGER);");
  }
  ...
  insertRecord() async {
    ...
    if (isInserted == false) {
      for (int i = 0; i < Globals.couponsData.length; i++) {


        String query =
            "INSERT INTO $tableName($colCoupon, $colQuantity) VALUES(?, ?)";

        Coupon data =  Coupon.data(data: Globals.couponsData[i]);
        List args = [data.name,data.quantity];
  ...
  updateRecord({required int id,required int quantity}) async {
    ...
    String query = "UPDATE $tableName SET $colQuantity= ? WHERE $colId = ?";

    List args =[--quantity, id];

    int? a = await db?.rawUpdate(query, args);
